# PROJECT RULES — WORDPRESS THEME DEVELOPMENT v2.16

> Theme: **tmnhanphat** · Text-domain: `tmnhanphat` · Function prefix: `tmnhanphat_`
> v2.16 = v2.15 + Products Section (Homepage, dưới Services): Panel Customizer riêng "Product Home" (`inc/customizer/products-customizer.php`, priority 34) — General/Heading/Description/Categories/Query/Card/Slider. Dữ liệu là POST THƯỜNG thuộc Category cha "Sản phẩm" (mặc định slug `san-pham`, chọn được trong Customizer), KHÔNG phải CPT. Category Navigation = các Sub Category, click tab đổi danh sách qua AJAX (`tmnhanphat_ajax_filter_products`, nonce-verified, term được xác thực thuộc cây category cha) — fallback không JS là link thẳng tới archive category. Slider dạng LƯỚI (`assets/js/components/products.js` — KHÁC `slider.js` trượt từng card): mỗi trang = Cols × Rows card (Desktop 3×2=6, Tablet 2×2=4, Mobile 1×1=1), PHP in card PHẲNG (no-JS = grid đầy đủ), JS gom trang theo breakpoint + dots/autoplay/loop/swipe. Thương hiệu/Tiêu chuẩn đọc từ post meta với META KEY CẤU HÌNH ĐƯỢC trong Customizer (mặc định `_tmnp_product_brand`/`_tmnp_product_standard`, nhập qua meta box "Thông tin sản phẩm" trên post — đổi key khi chuyển sang ACF, không sửa code); meta rỗng → ẩn dòng. Card dùng chung `template-parts/components/product-card.php` cho cả render đầu lẫn AJAX (DRY). Image size mới `tmnhanphat-product-card` 438×403 crop center — xem mục 29.
...
## 29. Products Section — sản phẩm là POST THƯỜNG theo Category, grid slider, AJAX filter

Panel **"Product Home"** (`inc/customizer/products-customizer.php`, priority 34, theo đúng mẫu mục 26) → Section **General/Heading/Description/Categories/Query/Card/Slider**. Helper: `tmnhanphat_products_defaults()`/`tmnhanphat_get_products_mod()`/`tmnhanphat_render_products_css_vars()` + nhóm query/category/meta trong `inc/template-functions.php`; template `template-parts/home/products.php`; card dùng chung `template-parts/components/product-card.php`; style `assets/css/components/products.css`; JS `assets/js/components/products.js`; AJAX `tmnhanphat_ajax_filter_products` trong `inc/ajax.php`.

**Nguồn dữ liệu — KHÔNG phải CPT:** sản phẩm là bài viết `post` thuộc cây Category cha "Sản phẩm" (mặc định slug `san-pham` — chọn lại được qua Customizer → Categories → Parent Category; Post Type cũng đổi được nếu sau này chuyển sang CPT). Đây là dữ liệu có sẵn trong `wp_posts` từ site cũ — không migrate, không tạo post type mới.

**Category Navigation:** hiển thị các **Sub Category** của Category cha (lọc Show Empty/Exclude, thứ tự theo term_id). Tab active nền xanh/chữ trắng. Click tab → AJAX đổi danh sách card (không reload); `href` của tab luôn là **link thật tới archive của category** nên khi JS tắt hoặc AJAX lỗi (catch → điều hướng theo href) vẫn dùng được — cùng nguyên tắc progressive enhancement toàn theme. Server xác thực `term_id` phải thuộc cây category cha (`tmnhanphat_is_valid_products_term()`) — không cho query category tuỳ ý. Trên mobile dải tab **cuộn ngang** (`overflow-x:auto`, ẩn scrollbar), không xuống nhiều dòng.

**Grid slider (khác `slider.js` của Services):** `slider.js` trượt TỪNG CARD; Products cần mỗi slide là 1 LƯỚI Cols × Rows (Desktop 3×2=6 bài, Tablet 2×2=4, Mobile 1×1=1 — đổi được qua Customizer → Query) nên có component riêng `products.js`. PHP in danh sách card **PHẲNG** trong track — không JS thì track là CSS Grid hiển thị đủ toàn bộ card (không mất nội dung); JS gom card thành từng `.tmnp-products-slider__page`, đổi breakpoint thì gom lại (giữ đúng card đầu đang xem), translateX theo %, dots theo số trang, autoplay/loop/pause-hover/swipe (Pointer Events)/keyboard, tôn trọng `prefers-reduced-motion`. Class `js-products-ready`/`.is-enhanced` CHỈ do JS gắn (bài học About Section).

**Thương hiệu/Tiêu chuẩn trên Card:** đọc từ post meta qua **meta key cấu hình được** trong Customizer → Card (mặc định `_tmnp_product_brand`/`_tmnp_product_standard`, nhập qua meta box "Thông tin sản phẩm" đăng ký cho `post` trong `inc/post-types.php`). Nếu sau này dùng ACF chỉ cần đổi meta key trong Customizer — không sửa code. Giá trị được `wp_strip_all_tags()` khi đọc (meta legacy trong DB có thể chứa HTML); **meta rỗng → ẩn nguyên dòng đó**, không render nhãn suông. Footer card: "Giá: Liên hệ" (Contact URL setting, mặc định `#` chờ Contact Page) + nút "Chi tiết" → permalink.

**Ảnh card:** image size riêng `tmnhanphat-product-card` **438×403 crop center** (`inc/images.php`) khớp Image Ratio mặc định `438-403` — card dùng `object-fit:cover`, không méo ảnh, lazy-load theo filter chung.
